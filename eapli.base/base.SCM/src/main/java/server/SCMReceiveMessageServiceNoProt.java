...
class SCMReceiveMessageServiceNoProt {
    ...
    public void callTCP() throws IOException {
        ...
        while (true) {
            ...
            new Thread(new TcpSrvSumThreadNoProt(cliSock, sIn, sOut)).start();
        }
    }
}

class TcpSrvSumThreadNoProt implements Runnable {
    private Socket s;
    private DataOutputStream sOut;
    private DataInputStream sIn;
    private final ProductionLineRepository plr = PersistenceContext.repositories().productionLines();

    public TcpSrvSumThreadNoProt(Socket cli_s, DataInputStream sIn, DataOutputStream sOut) {
        s = cli_s;
        this.sIn = sIn;
        this.sOut = sOut;
    }
...
/*            Arrays.fill(nack, 4, 5, (byte) sizeNack); // fill nack[4] and nack[5]
            Arrays.fill(nack, 6, nack.length - 1, Byte.parseByte(msgNack));*/
                ...
                String machineId = String.valueOf(idFloat);
                //size
