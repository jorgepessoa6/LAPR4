...
public class ProcessMessageSPMService implements Runnable {
    ...
    public ProcessMessageSPMService(ProductionLine productionLine, double intervalo) {
    ...
    public void run() {
        ...
        while (flag) {
            this.productionLine = productionLineRepository.productionLineByDesig(productionLine.identity());
            productionLine.setProductionLineStatus(ProductionLineStatus.ACTIVE);
            List<Message> validMessages = new ArrayList<>();
            Iterable<Message> messages = plr.allMessagesToProcessByProductionLine(productionLine);
            for (Message message : messages) {
                ...
                if (machine != null) {
                    ...
                    if (type.equals(Type.CONSUMO) || type.equals(Type
                            .ENTREGA_PRODUCAO) || type.equals(Type.ESTORNO)) {
                    ...
                    } else if (type.equals(Type.PRODUCAO)) {
                        validProduct(productionLine, arr[3], message);
                        validQuantity(productionLine, arr[4], message);
                        String lote = arr[5];
                    } else if (type.equals(Type.INICIO_ACTIVIDADE) || type.equals(Type.FIM_ATIVIDADE)) {
                        String ordemId = getOrder(arr, machine, productionLine, message);
                        ProductionOrder productionOrder = validOrder(productionLine, ordemId, message);
                    }
                    if (flag2) {
            ...
            try {
                System.out.println("\nSleeping");
                Thread.sleep((long) intervalo * 1000 * 60);
                System.out.println("\nWoke up");
            } catch (InterruptedException e) {
                System.out.println("\nInterrupted");
                this.productionLine.setProductionLineStatus(ProductionLineStatus.NON_ACTIVE);
                this.productionLineRepository.save(this.productionLine);
                flag=false;
            }
        }
    }
